...
public class SummingMethod {
    ...
    public static Map<String, Integer> summing(List<User> users) {
        return users.stream()
                .flatMap(user -> user.bills.stream().map(bill -> new Pair(user, bill)))
                .collect(Collectors.groupingBy(pair -> pair.user.getName(),
                        Collectors.summingInt(pair -> pair.bill.getBalance())));
    }
